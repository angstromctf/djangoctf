...
def validate_unique_team_name(name):
    """Checks if a team name is unique."""

    # Get all team with the specified team name
    teams = Team.objects.all().filter(name=name)
...
def validate_zip(zipcode):
    """Check if a ZIP code is valid (five characters long and numeric)."""
